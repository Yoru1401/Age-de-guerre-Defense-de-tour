...
class Menu {
public:
    Menu(std::string name, sf::RenderWindow* window) : window(window) {
        try {
            run();
        } catch (std::string s) {
            if (s==name) return;
    ...
    void addButton(Sprite sprite, Text *text, const std::function<void()> &action) {

        buttons.emplace_back(std::move(sprite), text, action);
    }
